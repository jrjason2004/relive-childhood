...
OBJECT-PLACE COHERENCE — the pairing must make real-world sense; a kid actually did THIS THING at THIS KIND OF PLACE. Never staple an iconic object onto a venue where it doesn't belong:
- Portable social fads (Silly Bandz, Pokémon cards, Tamagotchi, Beanie Babies, fidget spinners) travel anywhere kids gather — parks, pools, curbs, school pickup, diner booths. These are the go-to anchors for outdoor/public local places.
- Console/TV things (Wii Remote, N64 controller, Guitar Hero controller) only happen INSIDE a living room or basement of a residential house in a named local neighborhood — never outside a store, restaurant, or venue.
- Beyblades launch on flat ground at a friend's driveway/garage/playground blacktop; Razor scooters and Heelys live on sidewalks, driveways, and parking lots; pool toys at the pool; arcade tokens at the arcade; rental boxes inside the rental store.
- Retail products (a boxed game, a new toy in packaging) fit where they were bought or begged for — a toy aisle, checkout line, or just-walked-out storefront curb of the actual store type that sold them. A grocery store means grocery-era rituals (riding the cart, quarter machines, checkout candy), NOT holding a video game.
- WRONG: holding a boxed video game outside Harris Teeter; spinning Beyblades on a concert stage; a Wii Remote at a restaurant. If the iconic object doesn't naturally belong at the local place, either swap in a portable fad that does, or switch the scene to a residential/neighborhood setting where it does belong.
...
- "referenceQuery": the best Google Images search query to retrieve REAL PHOTOGRAPHS (not maps, logos, or graphics) of the PLACE ONLY — the named landmark/venue/setting. Be specific and include the city. NEVER include the toy/game/object in this query (no "Razor scooter", no "Wii" — the object comes from imagePrompt; this photo supplies the setting). If the place still exists but has been rebuilt or renovated since ${childhoodSpan}, bias the query toward how it looked then (e.g. add the decade, "vintage", "old", or "historic") so the results are period-accurate rather than the modern version.
