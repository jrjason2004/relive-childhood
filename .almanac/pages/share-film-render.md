...
2. **In-browser render — last resort.** `renderFilmInBrowser()` plays the clips into a canvas + `MediaRecorder` in real time. `finalFileExtRef` tracks the extension. In iOS Low Power Mode its dynamically created videos never play (no gesture unlock), so it can hang indefinitely — this is why it must never be the primary path.[@page]
